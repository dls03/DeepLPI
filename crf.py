...
class CRF(object):
    ...
    def parameter_learning(self, label, theta, sigma):
        def objective(theta, label, unary_potential, pairwise_potential, sigma):
            ...
            positive_spl_rate = 0.1
            negative_spl_rate = 1.0
            return -np.sum(np.log(q[label[positive_index[0]], positive_index[0]])) * negative_spl_rate  - np.sum(np.log(q[label[negative_index[0]], negative_index[0]])) * positive_spl_rate + np.sum(theta**2) / (2 * sigma)
        ...
        def gradient(theta, label, unary_potential, pairwise_potential, sigma):
            ...
            np.sum(q[1, positive_index[0]] * unary_potential[1, positive_index[0]]))) * negative_spl_rate + \
            ...
            np.sum(q[1, negative_index[0]] * unary_potential[1, negative_index[0]]))) * positive_spl_rate + \
            theta[0] / sigma
            ...
            np.sum(q[1, positive_index[0]] * pairwise_potential[1, positive_index[0]]))) * negative_spl_rate + \
            ...
            np.sum(q[1, negative_index[0]] * pairwise_potential[1, negative_index[0]]))) * positive_spl_rate + \
            theta[1] / sigma
